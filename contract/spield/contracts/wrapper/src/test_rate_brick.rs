#![cfg(test)]
//! # testcando.md §0 P0 — `brate_decrease_bricks_everything_including_exits`
//!
...
//! the whole protocol: **inflows *and* exits**.
//!
//! `set_max_apr_bps` cannot unstick it: that knob only widens the *upper* ceiling, and the
//! failure is the lower `current < last` guard. There is no valve for a decrease today.
//!
...
fn set_max_apr_bps_cannot_unstick_a_rate_decrease() {
    ...
    // The documented single-command production fix is useless here: widening the
    // annual ceiling to its maximum does nothing for the `current < last` guard.
    w.strategy().set_max_apr_bps(&u32::MAX);
    assert_rate_bricked(
        w.wrapper().try_claim_yield(&id),
        "set_max_apr_bps(u32::MAX) must NOT unstick a decrease — there is no valve for it",
    );

    // Recovery is only possible once Blend's rate climbs back to `last_rate`.
    w.strategy().force_rate(&(SCALAR_12 * 11 / 10));
