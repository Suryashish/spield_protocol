...
| 4b | The same retry, re-keyed on *exit status*, still double-executed: the CLI returns non-zero on `transaction submission timeout`, which happens when the transaction **was already included** and the client stopped waiting | **A write has three outcomes, not two — succeeded, failed, and indeterminate — and only the second is safe to retry.** Observed on a slow testnet: balances at exactly 2x. Retries are now restricted to errors that provably predate submission. |
| 4b | The exit-status fix for #4 **was not sufficient.** A `transfer` timed out *after* submitting, the retry ran, and the same 37,500,000 YT moved twice — surfacing three sections later as an alarming "claiming consumed YT" with nothing wrong in the contract | **A timeout is an `unknown`, not a `failure`.** We watched `set_timelock` report "transaction submission timeout" while having succeeded. Retry only on errors that provably happened *before* submission; treat everything else as ambiguous and stop. Any client that retries writes has this bug until proven otherwise. |
...
| 7 | `Sr::max_redeemable` was upgraded to call `strategy::available_liquidity`; the strategy was never redeployed. Every wiring check passed — the addresses were right — while the feature was dead on chain | **Version skew between two live deployments.** Pointing at each other and being able to *talk* to each other are different properties, and an address comparison cannot see the second. The deploy script now invokes each cross-boundary call and fails closed. |
...
Transaction-budget limits, what does not run on chain, and why: [`budget.md`](./budget.md).
