...
type NetworkConfig = { sources: Source[]; messenger: Hex; forwarder: string; stellarRpc: string; stellarPassphrase: string; stellarNetwork: string; stellarHorizon: string; stellarUsdcIssuer: string; iris: string; maxFee: bigint }
...
const NETWORKS: Record<Environment, NetworkConfig> = {
  mainnet: {
    ...
    stellarRpc: 'https://soroban-rpc.mainnet.stellar.org', stellarPassphrase: Networks.PUBLIC, stellarNetwork: 'PUBLIC', stellarHorizon: 'https://horizon.stellar.org', stellarUsdcIssuer: 'GA5ZSEJYB37JRC5AVCIA5MOP4RHTM335X2KGX3IHOJAPP5RE34K4KZVN', iris: 'https://iris-api.circle.com/v2/messages', maxFee: 0n,
    sources: [
  ...
  testnet: {
    ...
    stellarRpc: 'https://soroban-testnet.stellar.org', stellarPassphrase: Networks.TESTNET, stellarNetwork: 'TESTNET', stellarHorizon: 'https://horizon-testnet.stellar.org', stellarUsdcIssuer: 'GBBD47IF6LWK7P7MDEVSCWR7DPUWV3NY3DTQEVFL4NAT4AQH3ZLLFLA5', iris: 'https://iris-api-sandbox.circle.com/v2/messages', maxFee: 500n,
    sources: [
...
function App() {
  ...
  const [finality, setFinality] = useState<'standard' | 'fast'>('standard')
  ...
  const validAmount = amountUnits > 0n && balance !== null && amountUnits <= balance
  const ready = Boolean(evmAddress && stellarAddress && validAmount && !['attesting', 'forwarding'].includes(step))
  ...
  useEffect(() => {
    ...
    const fetchStellarBalance = async () => {
      ...
      try {
        ...
        if (active) { setStellarUsdcBalance(usdc?.balance ?? '0'); setStellarBalanceStatus('ready') }
      } catch { if (active) { setStellarUsdcBalance(null); setStellarBalanceStatus('error') } }
    }
  ...
  const changeEnvironment = (next: Environment) => {
    ...
    setBalance(null); setBalanceStatus('idle'); setStellarUsdcBalance(null); setStellarBalanceStatus('idle'); setAmount(''); setBurnHash(''); setRecipientToConfirm(''); setStep('idle'); setFinality('standard')
  ...
  const mintAndForward = async (message: Hex, attestation: Hex, recipient: string) => {
    ...
    const transaction = new TransactionBuilder(account, { fee: '10000000', networkPassphrase: config.stellarPassphrase })
  ...
  const bridge = async (recipient: string) => {
    try {
      ...
      const burn = encodeFunctionData({ abi: cctp, functionName: 'depositForBurnWithHook', args: [amountUnits, 27, forwarder, source.usdc, forwarder, config.maxFee, finality === 'fast' ? 1000 : 2000, hookData] })
      ...
      setNotice('Approve USDC in your EVM wallet.'); setStep('approving')
      ...
      setNotice('Calldata recipient verified. Confirm the CCTP burn. USDC will be minted to the Stellar Forwarder and forwarded atomically to your Stellar address.'); setStep('burning')
  ...
  return <main className="shell">
    ...
    <section className="bridge-grid">
      <div className="bridge-card">
        ...
        <div className="route-block">
          ...
          {menuOpen && <div className="network-menu">{config.sources.map((item) => <button key={item.chainId} className={item.chainId === source.chainId ? 'selected' : ''} onClick={() => void selectSource(item)}><span className={`token token-${item.short.toLowerCase()}`}>{item.logo}</span>{item.name}<small>Domain {item.domain}</small></button>)}</div>}
        ...
        <div className="route-block destination"><div className="route-label">TO</div><div className="network-picker static"><span className="token stellar">✦</span><span><b>Stellar</b><small>Native USDC • {environment === 'testnet' ? 'Testnet' : 'Mainnet'}</small></span><span className="route-lock">↗</span></div><div className="receive">You receive <b>{amount || '0.00'} USDC</b><span>1:1 native transfer</span></div></div>
        <div className="mode"><button className={finality === 'standard' ? 'active' : ''} onClick={() => setFinality('standard')}><b>Standard</b><span>Economical finality</span></button><button disabled={!source.fast} className={finality === 'fast' ? 'active' : ''} onClick={() => source.fast && setFinality('fast')}><b>Fast</b><span>{source.fast ? 'When available' : 'Unavailable on this source'}</span></button></div>
        ...
        <button className="bridge-button" disabled={!ready} onClick={() => void prepareBridge()}>{step === 'complete' ? 'Transfer complete' : step === 'error' ? 'Verify recipient & try again' : !evmAddress ? 'Connect EVM wallet first' : !stellarAddress ? 'Connect Stellar wallet first' : !validAmount ? 'Enter a valid amount' : `Bridge ${routeSummary}`}</button>
      </div>
    ...
    {recipientToConfirm && <div className="recipient-modal" role="dialog" aria-modal="true" aria-labelledby="recipient-title"><div className="recipient-dialog"><p className="eyebrow">FINAL RECIPIENT CHECK</p><h2 id="recipient-title">This address will receive your Stellar USDC.</h2><p>It is encoded permanently in the CCTP burn. Confirm it character by character in Freighter before continuing.</p><div className="recipient-address"><span>CONNECTED FREIGHTER ADDRESS</span><code>{recipientToConfirm}</code></div><div className="recipient-actions"><button className="cancel" onClick={() => setRecipientToConfirm('')}>Cancel</button><button className="confirm" onClick={() => { const recipient = recipientToConfirm; setRecipientToConfirm(''); void bridge(recipient) }}>Confirm recipient & bridge</button></div></div></div>}
  </main>
}
