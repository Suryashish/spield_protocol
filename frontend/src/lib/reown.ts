...
/**
 * Reown AppKit — the EVM and Solana wallet connector used only for the CCTP bridge. The app's
 ...
 * is imported for its side effect from `ReownContext`. `isReownConfigured` lets
 * the UI degrade gracefully when no project id is set:
...
if (isReownConfigured) {
  createAppKit({
    ...
    networks: NETWORK_KEY === 'mainnet'
      ? [mainnet, base, arbitrum, optimism, polygon, avalanche, solana]
      : [sepolia, optimismSepolia, arbitrumSepolia, baseSepolia, unichainSepolia, lineaSepolia, solanaDevnet],
